...
int nb_obj_in_core_table;  /* total number of objects (samples) */
                           /* The first nb_obj_in_core_table - nb_obj_in_validation_table are be used for constructing trees */
...
int save_ensemble_while_growing=1;  /* 1 to write tree data to file, 0 not to */
int store_ensemble=1;


/* parametres generaux pour la classification */

int nb_classes; /* nombre de classes pour la classification */  /* Not used except for write_one_tree that has been deactivated in dynGENIE3! */
...
// If the tree is built using a fixed set of K attributes, construct the PRESELECTED_ATTRIBUTES array
  ...
  for(int ith_preselected = 0; ith_preselected < rf_k; ++ith_preselected) {
    int lleft = 0;
    int rright = nb_attributes;
    float x_rand = unif_rand()*cumulative_priors[nb_attributes-1]; /* in case priors are not normalized */
    ...
    if(x_rand<cumulative_priors[0]){
      ...
      while(lleft != rright -1){
        ...
        else if(cumulative_priors[mid] >= x_rand){
           rright = mid;
        }
      }
      random_att_pos = rright;
    }   
    PRESELECTED_ATTRIBUTES[ith_preselected] = random_att_pos;
  }
 ...
 * Removed the part that takes away an attribute (ie. a gene) once it has
 ...
 * Those preselected attributes are specified in the array PRESPECIFIED_ATTRIBUTES
 */
...
/* thus v_tot would be the variance before and info the reduction_of_variance */
...
// Ouput arguments (memory must have been generated within the R and associated pointers passed):
...
void R_init_dynGENIE3(DllInfo *info) {
