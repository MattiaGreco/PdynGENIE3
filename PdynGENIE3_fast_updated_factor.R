...
# alpha = (1 - (1/2) sigma(2:end) LAMBDA / sigma(1:end-1) ] delta_t
# In the computation, if we first center + normalize the quantities,
# the sigmas can be ignored
# Note: in the definition of Lambda in terms of scalar products, it is best
# to divide each scalar product by the number of terms as that then also allows
# one to drop sigmas. The interpretation via correlation coefficients is useful.
different_time_corr <- function(TS.data, time.points){
  ...
  for(kk in 1:num.exp){
    ...
    tot_weight <- tot_weight + num.time.points - 1
    time.diff.current <- current.time.points[2:num.time.points] - current.time.points[1:(num.time.points-1)]
    # For computational efficiency, compute the time truncated driver data, centered and normalized by standard deviation
    drivers_standardized <- as.matrix(current.timeseries[,1:(num.time.points-1)]) 
    drivers_standardized <- drivers_standardized - rowMeans(drivers_standardized)
    work_matrix <- drivers_standardized*drivers_standardized
    drivers_standardized <- t(drivers_standardized/sqrt(rowMeans(work_matrix)))  # no (n-1) correction for unbiased estimate of variance
                                                                                 # transpose useful for later operations

    for(ii in 1:num.genes) { # loop over targets, correlations weighted by number of kept time points
      this_target_standardized <- as.numeric(current.timeseries[ii,2:num.time.points])
      this_target_standardized <- this_target_standardized - mean(this_target_standardized)
      this_target_standardized <- this_target_standardized / sqrt(mean(this_target_standardized*this_target_standardized))
      work_matrix <- this_target_standardized*drivers_standardized
      Lambda[ii,] <- Lambda[ii,] + colSums(work_matrix)  # Sum rather than Means to weight more longer time series
      # Now for calculating alphas
      factor <- 0.5*sqrt(var(current.timeseries[ii,2:num.time.points]) / var(current.timeseries[ii,1:(num.time.points-1)]) )
      this_Lambda_ii_ii <- mean(work_matrix[,ii])
      A[ii, kk] <- (num.time.points-1)*(1-factor*this_Lambda_ii_ii)/median(time.diff.current)   # (num.time.points-1) for weights
    } 
  }  # end of loop over k (number of time series experiments)
  Lambda <- Lambda / tot_weight
  alphas <- rowSums(A) / tot_weight

  results <- list("Lambda_target_i_driver_j" = Lambda, "alphas" = alphas)
